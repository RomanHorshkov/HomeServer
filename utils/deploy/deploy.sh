...
echo "chown /srv/"$USER_GROUP_NAME" to "$USER_GROUP_NAME":"$USER_GROUP_NAME""
...
exec sudo -u "$USER_GROUP_NAME" -H \
  ...
  bash -c "umask 027; cd /srv/home_server; exec '$dest/$bin' 3491 >> /srv/home_server/logs/server.log 2>&1"
