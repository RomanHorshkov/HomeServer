...
int listener_check_incoming_clients(Listener_t **listener, struct sockaddr_storage *client_addr, int *client_fd)
{
    /* TO DO:
    ...
    for (int i = 0; i < MAX_LISTENERS; i++)
    {
        ...
        client_fd_tmp = accept(listener_fd_tmp,
...
static int start_listener(struct addrinfo *server_info_out, Listener_t **listener_ptr)
{
    ...
    {
        /* Create a socket */
        int listener_socket_fd = socket(server_info_out->ai_family, server_info_out->ai_socktype, server_info_out->ai_protocol);
        ...
        /* Bind the socket */
        else if(bind(listener_socket_fd, server_info_out->ai_addr, server_info_out->ai_addrlen) == -1)
        {
        ...
        else
        {
            ...
            (*listener_ptr)->listeners_fds[(*listener_ptr)->active_listeners_no++] = listener_socket_fd;
