 ...
 *
 * S2 note on the send path: partial writes and EAGAIN are handled with a
 * bounded poll(POLLOUT) wait inside the operator thread. The full
 * EPOLLOUT re-arm (parking the unsent tail and resuming from the reactor)
 * is the §9.2 end state and lands with the streaming slice — for API-sized
 * responses on localhost-proxied connections the bounded wait is correct
 * and loud on timeout.
 *
...
int response_writer_error(client_t* cli, uint16_t status)
{
    ...
    switch(status)
    {
        case 404u: body = body_404; break;
        case 400u: body = body_400; break;
        default:   status = 500u; break;
    }
...
static int _send_all(client_t* cli, const char* data, size_t len)
{
    ...
    while(sent < len)
    {
        ...
        if(n < 0 && errno == EAGAIN)
        {
            ...
            EML_ERROR(LOG_TAG, "fd %d: POLLOUT wait failed (pr=%d revents=0x%x) after %zu/%zu B", cli->ctx.fd, pr,
                      (unsigned)pfd.revents, sent, len);
            return STATUS_FAILURE;
        }
